...
saveDFtoFiles <- function(x, y) {
  ...
  write.csv(temp.df, file = paste("../text/mRNA/SigCodingmRNA_log", x, ".csv", sep = ""))
}
...
write.csv(summaryDF, file = "../text/mRNA/significantCodingmRNAByTCGAtypeslog.csv")
...
write.csv(x = mRNANamesPredictives, file = "../text/mRNA/union_of_predictive_CodingmRNA_through_tumors_list_log.csv")
