...
### Get phenotypes
pheno <- as.data.frame(colData(rse_scaled))
...
TCGANeg <- ExpressionSet(assayData <- rseCountsGene,
                          phenoData = AnnotatedDataFrame(pheno),
                          featureData= AnnotatedDataFrame(ann),
                          MIAME = miame)
annotation(TCGANeg) <- "CAT.FANTOM"
...
save(TCGANeg, file="objs/TCGANeg.rda")
